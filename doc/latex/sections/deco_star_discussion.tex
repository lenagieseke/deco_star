...
Table~\ref{table:analysis} lists the control mechanisms categorized in~\Cref{sec:taxo_control_mechanism} for the above-discussed publications. We considered the same publications in Table~\ref{table:taxo_controlmechanism} for interrelating the control mechanisms to the control characteristics. Due to the diversity of the underlying methods, and the different design goals of the considered body of work, we believe this is a representative summary.
...
However, because creation techniques and design spaces are open, it could lead to manual and tedious creation requirements for patterns. However, recent work such as~\citeauthor*{hsu_2020_aef}~\cite{hsu_2020_aef} in regard to element arrangements and the incorporation of procedural creation principles for automatic fillings into a data-driven process by \citeauthor*{kazi_2012_vit}~\cite{kazi_2012_vit} and \citeauthor*{xing_2014_apr}~\cite{xing_2014_apr} are promising directions. \citeauthor*{hsu_2020_aef}~\cite{hsu_2020_aef} offer transparent navigation and an open design space within the topic of element arrangements as with a brush an artist is free to draw almost anything - depending only on one's personal drawing capabilities. For creative pattern generation elements are often carefully connected, creating a hierarchy of structures, such as for ornaments. Hence, element arrangements can only provide a subset - albeit an important one - of the design space needed for creative pattern generation. 
