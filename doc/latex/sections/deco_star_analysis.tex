...
% MF: Is this a figure? then reference it as Figure … , if it is a table, make it a table environment, as well?
% Can we spell out more of the heading? I find myself wondering what Hand. , Exemp. mean ..
%  (Always put the label inside (or after?) the caption. Placed before, the reference will give the section number instead of the figure number)
...
Table~\ref{table:analysis} lists for the above discussed control mechanisms the in~\Cref*{sec:taxo_control_mechanism} discussed categories. For interrelating the control mechanisms to the control characteristics, we considered the publications in Table~\ref{table:analysis}. Due to the diversity of the underlying methods and the different design goals of the considered body of work, we believe this to be a representative summarization.
...
% MF: Just taking note of this now: are we too criticial / negative? „little effort has been made“ would hurt me, say, if Gieseke et al. would show up in this category ;-)
