...
The following survey contributes with analyzing control mechanisms in the state of the art for creative pattern generation from the perspective of an artist. Techniques are investigated as a whole, from required configuration input, over creative tasks, to performance times. The analysis of the control mechanisms is directly taken from the authors' descriptions. 

Publications are clustered by the design features they enable. For detailed explanations of the design festures, refer to~\Cref{sec:design}. If a reference belongs to several design categories, it is discussed in detail in the most fitting area and then breifly referenced in all other applicable areas. Techniques are considered procedural if not indicated otherwise.

\subsection{Common Control Mechanisms}
\label{subsec:analysis_commo_machanism}


Within the above design areas, control mechanisms are manifold and well distributed over all areas. Because of that, we start by briefly summarize the most common control mechanisms and their role in creative pattern generation processes before turning to the state of the art.


\subsubsection{Example-Based Control}
\label{subsubsec:analysis_example}

On of the most prominently investigated techniques for texturing methods are example-based approaches. Example-based control mechanisms compute a separate output based on a given example, e.g., an image and provide a \textit{goal-oriented control}. The motivation behind using these techniques is mainly to generate a specific and predictable output as efficiently as possible. Example-based and inverse approaches have a long history in the control of procedural representations. 
...
\label{subsubsec:analysis_shapes}
...
% TODO: What to do about it in the table?

\subsubsection{Vector Fields}
\label{subsubsec:analysis_fields}
...
\subsubsection{Curves and Brushing}
...
Painting-tool-like methods like brushing create output along curves but do so directly without taking an a priori completed curve into consideration, as if using a spray can or a brush. Painting techniques usually include a brush diameter, hence the size of the area to be filled along the curve.

\subsubsection{Element Placement}
\label{subsubsec:analysis_elements}
...
These common control mechanisms are used in various implementations as we show in the following review, which is organized by design areas. Only work that goes beyond filling a space, are included in the summarizing table (~\ref{table:analysis}).
...
Element arrangements have individual and unconnected visual entities as smallest unit. The elements themselves usually come from input data, such as vector files. In its broadest sense, the underlying distribution models for the arrangements can be seen as a procedural model. Even though there are no generative rules, characteristics of the discrete elements and their distribution can often be parametrized, and changes can be automatically processed and reproduced in the output.
...
\subsection{Curves, Lines and Brushing}
\label{subsec:analysis_curves}

Curves, lines and brushing are not only used indirectly control tool, e.g., for creating a decorative frame, but also as a visual element in itself. 
...
\majortodo[inline]{Add work from 2018 on to the discussion.}
