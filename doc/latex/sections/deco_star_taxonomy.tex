...
\bb{Should we merge this with the previous section and call it Terminology and Taxonomy of Control Mechanisms?}\legie{What would be the benefit of it? Saving space?}

% \bb{I would significantly simplify this section. Each could be one paragraph \eg timeline could look like: \textbf{When? - Timeline} defines when can input be given and at what time in the creation process is the control executed. It can be input is given \textit{before}, \textit{during} the creation process, when parts of the results are already visible, or \textit{after} the creation process. The result is visible to the artist and can be adjusted retrospectively. Have a look at the timeline section and see how many times it says "creative process". Also - I do not understand the images in this section and I would probably remove most of them. I feel the concepts are obvious \eg global-local, abstract-concrete, before-during-after and they do not need an image. Also, we should have the list of papers that belong to each category in Table~1 somewhere - in Appendix?}
...
% \bb{Below is a NEW version of how I would simplify it. The original version is commented}
